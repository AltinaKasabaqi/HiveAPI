...
namespace HiveAPI.Services.WorkSpaceServices
{
    public class WorksService : IWorksService
    {
        ...
        {
            var user = await _context.Users.FirstOrDefaultAsync(u => u.UserId == ws.UserId);
            if (user == null)
            {
        ...
        public async Task<List<WorkSpace>> GetWorkSpacesByUserId(int Id)
        {
            var workSpaceWithUser = await _context.WorkSpaces
                .Include(ws => ws.User) // Lidh hapësirën e punës me përdoruesin
                .Where(ws => ws.UserId == Id) // Filtroni hapësirën e punës me ID specifik
